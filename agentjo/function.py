...
Can also be done automatically by providing docstring with input variable names in external_fn
        ...
        - llm: Function. The llm parameter to pass into llm_parser
        - **kwargs: Dict. Additional arguments you would like to pass on to the llm_parser (such as llm)
    ...
    def _prepare_function_kwargs(self, *args, **kwargs):
        ...
        # extract out only variables listed in variable_list from kwargs not including shared_variables
        function_kwargs = {key: value for key, value in kwargs.items() if key in self.variable_names and key != "shared_variables"}
...
class Function(BaseFunction):
    def __init__(self, *args, **kwargs):
        ...
        if self.fn_name is None:
            ...
            # otherwise, generate name out
                ...
                res = self.llm_parser(system_prompt = "Output a function name to summarise the usage of this function.",
    ...
    def __call__(self, *args, **kwargs):
        ...
        # If llm-based function, do the function. 
        if self.external_fn is None:
            res = self.llm_parser(system_prompt = self.fn_description,
                            user_prompt = str(function_kwargs),
...
class AsyncFunction(BaseFunction):
    def __init__(self, *args, **kwargs):
        ...
        # ensure llm and llm parser are async
    ...
    async def async_init(self): 
        ''' This generates the name for the function using llm_parser '''
        if self.fn_name is None:
            res = await self.llm_parser_async(system_prompt = "Output a function name to summarise the usage of this function.",
                              user_prompt = str(self.fn_description),
                              output_format = {"Thoughts": "What function does", "Name": "Function name with _ separating words that summarises what function does"},
                             llm = self.llm,
                             **self.kwargs)
    ...
    async def __call__(self, *args, **kwargs):
        ...
        - shared_varables: Dict. Default: empty dict. The variables which will be shared between functions. Only passed in if required by function 
        - *args: Tuple. Unnamed input variables of the function. Will be processed to var1, var2 and so on based on order in the tuple
        - **kwargs: Dict. Named input variables of the function. Can also be variables to pass into llm_parser_async
        ...
        # If llm-based function, do the function. 
        if self.external_fn is None:
            res = await self.llm_parser_async(system_prompt = self.fn_description,
                            user_prompt = str(function_kwargs),
                            output_format = self.output_format,
                            llm = self.llm,
                            shared_variables = shared_variables,
