...
class BaseAgent:
    def __init__(self, agent_name: str = 'Helpful Assistant',
        ...
        - llm: Function. The llm parameter that gets passed into Function/llm_parser
        
        Inputs (optional):
        - **kwargs: Dict. Additional arguments you would like to pass on to the llm_parser function
...
class Agent(BaseAgent):
    def __init__(self, *args, **kwargs):
        ...
        self.default_memory = Memory(top_k = 5, mapper = lambda x: x.fn_name + ': ' + x.fn_description, approach = 'retrieve_by_ranker', llm = self.llm, llm_parser = self.llm_parser, llm_parser_async = self.llm_parser_async)
        ...
        if self.default_to_llm:
            self.assign_functions([Function(fn_name = 'use_llm', 
                                        fn_description = 'For general tasks. Used only when no other function can do the task. Try to use it in a modular manner and perform one part of the task at a time.', 
                                        is_compulsory = True,
    ...
    def query(self, query: str, output_format: dict, provide_function_list: bool = False, task: str = ''):
        ...
        res = self.llm_parser(system_prompt = '',
    ...
    def assign_functions(self, function_list: list):
        ...
        for function in function_list:
            ...
            # do automatic conversion of function to Function class
...
End Task if Assigned Task is completed.''',
    ...
    def reply_user(self, query: str = '', stateful: bool = True, verbose: bool = False):
        ...
        if stateful:
            self.add_subtask_result('Replying User Query: ' + str(my_query), res)
...
class AsyncAgent(BaseAgent):
    def __init__(self, *args, **kwargs):
        ...
        self.default_memory = AsyncMemory(
            ...
            llm_parser_async = self.llm_parser_async
        )
        if self.memory_bank is None:
            self.memory_bank = {'Function': self.default_memory}
            self.memory_bank['Function'].reset()
    ...
    async def query(self, query: str, output_format: dict, provide_function_list: bool = False, task: str = ''):
        ...
        res = await self.llm_parser_async(system_prompt = '',
        user_prompt = user_prompt,
        output_format = output_format, 
        verbose = self.debug,
        llm = self.llm,
...
End Task if Assigned Task is completed.''',
    ...
    async def reply_user(self, query: str = '', stateful: bool = True, verbose: bool = False):
        ''' Generate a reply to the user based on the query / agent task and subtasks completed
        If stateful, also store this interaction into the subtasks_completed
        If verbose is given, can also override the verbosity of this function'''
